...
function QueryBuilder(model, opts, params) {
  ...
  this.getIncludes = function (modelForIncludes, fieldNamesRequested) {
    return _.values(modelForIncludes.associations).filter(function (association) {
      return (!fieldNamesRequested || fieldNamesRequested.includes(association.as)) && ['HasOne', 'BelongsTo'].includes(association.associationType) &&
