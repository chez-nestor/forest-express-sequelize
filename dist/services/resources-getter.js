...
function ResourcesGetter(model, opts, params) {
  ...
  var fieldNamesRequested = function () {
    ...
    //         the client.
    return _.union([primaryKey], params.fields[model.name].split(','), associationsForQuery);
  }();

  var searchBuilder = new SearchBuilder(model, opts, params, fieldNamesRequested);
  ...
  function handleFilterParams() {
    ...
    _.each(params.filter, function (values, key) {
      ...
      values.split(',').forEach(function (value) {
        var condition = {};
        condition[key] = new OperatorValueParser(opts).perform(model, key, value, params.timezone);
